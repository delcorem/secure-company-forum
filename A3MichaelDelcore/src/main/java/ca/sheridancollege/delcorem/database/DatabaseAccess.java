...
public class DatabaseAccess {
	...
	public void insertUser(User user) {
		...
		LocalDate discussionDate = LocalDate.now();
		LocalTime discussionTime = LocalTime.now();
		MapSqlParameterSource namedParameters = new MapSqlParameterSource();
		String query = "INSERT INTO discussion(email, userdiscussion, discussiondate, discussiontime) VALUES (:email, :userdiscussion, :discussiondate, :discussiontime)";
		namedParameters.addValue("email", user.getEmail());
		namedParameters.addValue("userdiscussion", user.getUserDiscussion());
		namedParameters.addValue("discussiondate", discussionDate);
		namedParameters.addValue("discussiontime", discussionTime);
		int rowsAffected = jdbc.update(query, namedParameters);
		if (rowsAffected > 0)
			System.out.println("Inserted discussion into database.");
	}
